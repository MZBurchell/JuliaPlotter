...
def on_click(event):
    ...
    if event.button is MouseButton.LEFT and event.inaxes == ax1:
        ...
        axesHandle.set
        ax2.imshow(julia_gen(complex(-2.0 + ((2.5/3000) * event.xdata), -1.5 + event.ydata/1000), 500, 500, -1.5, -1.5, 3.0, 3.0, 10, 100), cmap='inferno')
...
plt.tick_params(
    ...
    labelleft=False,         
    labelbottom=False)
axesHandle = ax2.imshow(julia_gen(complex(-0.1, 0.65), 500, 500, -1.5, -1.5, 3.0, 3.0, 10, 100), cmap='inferno')
